...
def create_scatter_map(
    ...
    output_file: str = "scatter_map.svg",
    ...
    region_data : dict of str to int
        Dictionary mapping region names to total bytes downloaded
    coordinates : dict of str to dict
        Dictionary mapping region codes to coordinate dictionaries with 'latitude' and 'longitude'
    output_file : str, optional
        Output filename for the map, by default 'scatter_map.svg'
    dandiset_ids : list of str, optional
        List of dandiset IDs being visualized (for title), by default None

    Examples
    --------
    >>> region_data = {'US/California': 1000000000, 'DE/Berlin': 500000000}
    >>> coordinates = {'US/California': {'latitude': 36.7, 'longitude': -119.7}}
    >>> create_scatter_map(region_data, coordinates, 'my_map.svg')
    """
    ...
    for region, bytes_downloaded in region_data.items():
        if region in coordinates:
            ...
            if lat is not None and lon is not None and float(lat) > -60:
                fill_color, edge_color, size = get_point_color_and_size(
                    bytes_downloaded
                )
                plot_data.append(
                    {
...
def main() -> None:
    ...
    >>> # python create_scatter_map.py --dandiset 000026,000409 --output my_map.svg
    """
    parser = argparse.ArgumentParser(
        description="Create scatter plot map of DANDI downloads by region"
    )
    parser.add_argument(
        "--output",
        "-o",
        default="output/scatter_map.svg",
