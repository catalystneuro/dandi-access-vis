...
def create_choropleth(country_data, log_scale=True, output_file='choropleth_map.svg', dandiset_ids=None):
...
def main():
    ...
    if dandiset_ids:
        if len(dandiset_ids) == 1:
            print(f"Loading and processing country data for dandiset {dandiset_ids[0]}...")
        else:
            print(f"Loading and processing country data for {len(dandiset_ids)} dandisets...")
    else:
    ...
    create_choropleth(country_data, log_scale=args.log_scale, output_file=args.output, dandiset_ids=dandiset_ids)
